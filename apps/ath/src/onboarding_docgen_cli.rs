...
use athanor_app::{
    ...
    DocumentationOnboardingCurrentInspection, DocumentationOnboardingManifestInspection,
    DocumentationOnboardingOperationOptions, DocumentationOnboardingPublicationReport,
    DocumentationOnboardingPublicationStatus, DocumentationOnboardingValidationInspection,
    DocumentationProfile, generate_documentation_onboarding_with_composition_cancellable,
    inspect_documentation_onboarding_current, inspect_documentation_onboarding_manifest,
    inspect_documentation_onboarding_validation,
};
...
pub(crate) enum Command {
    /// Generate deterministic onboarding documentation from one exact committed snapshot.
    GenerateOnboarding {
        #[arg(default_value = ".")]
        path: PathBuf,
        #[arg(long)]
    ...
    /// Inspect the current onboarding documentation generation.
    Onboarding {
        #[command(subcommand)]
...
pub(crate) fn parse(args: &[String]) -> Result<Option<Command>> {
    ...
    let owned = matches!(
        ...
        Some("generate-onboarding" | "onboarding")
    );
    if !owned {
        return Ok(None);
    }
    ...
    match command {
        Command::GenerateOnboarding {
            ...
            let mut generation = Box::pin(
                generate_documentation_onboarding_with_composition_cancellable(
                    DocumentationOnboardingOperationOptions {
                        root: path,
                        request,
                        force,
                    },
            ...
            let report = tokio::select! {
                ...
                signal = &mut ctrl_c => {
                    signal.context("failed to listen for onboarding generation cancellation")?;
        ...
        Command::Onboarding {
            command: OnboardingCommand::Current { path, json },
        } => render_current(&inspect_documentation_onboarding_current(path)?, json)?,
        Command::Onboarding {
            command: OnboardingCommand::Manifest { path, json },
        } => render_manifest(&inspect_documentation_onboarding_manifest(path)?, json)?,
        Command::Onboarding {
            command: OnboardingCommand::Validation { path, json },
        } => render_validation(&inspect_documentation_onboarding_validation(path)?, json)?,
    }
    Ok(())
}

fn render_generation(report: &DocumentationOnboardingPublicationReport, json: bool) -> Result<()> {
    if json {
        println!("{}", serde_json::to_string_pretty(report)?);
        return Ok(());
    }
    match report.status {
        DocumentationOnboardingPublicationStatus::Published => println!(
            "published onboarding documentation generation {} from snapshot {}",
            report.generation, report.snapshot
        ),
        DocumentationOnboardingPublicationStatus::UpToDate => println!(
            "onboarding documentation generation {} is already up to date for snapshot {}",
            report.generation, report.snapshot
        ),
    }
...
fn render_current(report: &DocumentationOnboardingCurrentInspection, json: bool) -> Result<()> {
    if json {
        println!("{}", serde_json::to_string_pretty(report)?);
    } else {
        println!(
            "current onboarding documentation generation {} for snapshot {}",
            report.current.generation, report.current.snapshot
        );
        println!("profile: onboarding");
...
fn render_manifest(report: &DocumentationOnboardingManifestInspection, json: bool) -> Result<()> {
    if json {
        println!("{}", serde_json::to_string_pretty(report)?);
        return Ok(());
    }
    println!(
        "onboarding documentation manifest {} for snapshot {}: {} artifacts",
...
fn render_validation(
    ...
    println!(
        "onboarding documentation validation for snapshot {}: valid",
