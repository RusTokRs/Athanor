...
use athanor_app::{
    ...
    graph_fba_dependencies_with_operation_context, graph_fba_module_with_operation_context,
    graph_fba_port_with_operation_context, graph_fba_violations_with_operation_context,
    graph_ffa_surface_with_operation_context, graph_ffa_violations_with_operation_context,
    graph_page_builder_consumer_with_operation_context,
    graph_page_builder_provider_with_operation_context,
    graph_page_builder_violations_with_operation_context,
    rustok_architecture_context_with_operation_context, rustok_fba_audit_with_operation_context,
    rustok_ffa_audit_with_operation_context, rustok_page_builder_audit_with_operation_context,
};
    ...
    match command {
        Command::ArchitectureContext { intent, flags } => {
            ...
            let report = await_drained_operation(
                cancellation,
                rustok_architecture_context_with_operation_context(options, &operation),
            )
            .await?;
            if flags.json {
                println!("{}", serde_json::to_string_pretty(&report)?);
            } else {
                legacy::print_rustok_architecture_context_bridge(&report);
            }
        }
        Command::FfaAudit(flags) => {
            ...
            let report = await_drained_operation(
                cancellation,
                rustok_ffa_audit_with_operation_context(
            ...
            if flags.json {
                ...
                legacy::print_rustok_ffa_audit_bridge(&report);
            }
        }
        Command::FbaAudit(flags) => {
            ...
            let report = await_drained_operation(
                cancellation,
                rustok_fba_audit_with_operation_context(
            ...
            if flags.json {
                ...
                legacy::print_rustok_fba_audit_bridge(&report);
            }
        }
        Command::PageBuilderAudit(flags) => {
            ...
            let report = await_drained_operation(
                cancellation,
                rustok_page_builder_audit_with_operation_context(
            ...
            if flags.json {
                ...
                legacy::print_rustok_page_builder_audit_bridge(&report);
            }
        }
        Command::FfaSurface {
            module,
            ...
            let report = await_drained_operation(
                cancellation,
                graph_ffa_surface_with_operation_context(
        ...
        Command::FfaViolations(flags) => {
            ...
            let report = await_drained_operation(
                cancellation,
                graph_ffa_violations_with_operation_context(
        ...
        Command::FbaModule { module, flags } => {
            ...
            let report = await_drained_operation(
                cancellation,
                graph_fba_module_with_operation_context(
        ...
        Command::FbaPort {
            ...
            let report = await_drained_operation(
                cancellation,
                graph_fba_port_with_operation_context(
                    GraphFbaPortOptions {
                        root: flags.path,
                        module,
                        port,
        ...
        Command::FbaDependencies(flags) => {
            ...
            let report = await_drained_operation(
                cancellation,
                graph_fba_dependencies_with_operation_context(
        ...
        Command::FbaViolations(flags) => {
            ...
            let report = await_drained_operation(
                cancellation,
                graph_fba_violations_with_operation_context(
...
fn render_ffa_graph(report: &athanor_app::RustokFfaGraph, json: bool) -> Result<()> {
    if json {
        ...
        legacy::print_rustok_ffa_graph_bridge(report);
    }
    Ok(())
}
...
fn render_fba_graph(report: &athanor_app::RustokFbaGraph, json: bool) -> Result<()> {
    if json {
        ...
        legacy::print_rustok_fba_graph_bridge(report);
    }
    Ok(())
}
...
fn render_page_builder_graph(
    ...
    if json {
        ...
        legacy::print_rustok_page_builder_graph_bridge(report);
    }
    Ok(())
}
