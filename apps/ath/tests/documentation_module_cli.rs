...
#[test]
fn module_documentation_help_exposes_exact_snapshot_and_inspection() {
    for args in [
        vec!["docs", "generate-module", "--help"],
        vec!["docs", "module", "current", "--help"],
        vec!["docs", "module", "manifest", "--help"],
        vec!["docs", "module", "validation", "--help"],
    ] {
        let output = run(&args);
        assert!(
...
#[test]
fn exact_snapshot_module_generation_and_inspection_round_trip_through_binary() {
    ...
    assert!(
        ...
        "module generation stderr: {}",
        stderr(&generated)
    );
    let generation: Value =
        serde_json::from_slice(&generated.stdout).expect("module generation JSON report");
    ...
    let current = json_command(&["docs", "module", "current", root_arg, "--json"]);
    assert_eq!(current["current"]["snapshot"], snapshot);
    assert_eq!(current["current"]["profile"], "module");
    assert_eq!(current["current"]["generation"], generation["generation"]);

    let manifest = json_command(&["docs", "module", "manifest", root_arg, "--json"]);
    assert_eq!(manifest["manifest"]["snapshot"], snapshot);
    assert_eq!(manifest["manifest"]["profile"], "module");
    assert_eq!(
    ...
    assert_eq!(validation["report"]["profile"], "module");
...
#[test]
fn missing_exact_snapshot_fails_without_module_publication() {
...
fn fixture_root(label: &str) -> PathBuf {
    ...
    let root = std::env::temp_dir().join(format!(
        "athanor-module-cli-{label}-{}-{nonce}",
        std::process::id()
    ));
