...
#[test]
fn onboarding_documentation_help_exposes_exact_snapshot_and_inspection() {
    for args in [
        vec!["docs", "generate-onboarding", "--help"],
        vec!["docs", "onboarding", "current", "--help"],
        vec!["docs", "onboarding", "manifest", "--help"],
        vec!["docs", "onboarding", "validation", "--help"],
    ] {
        let output = run(&args);
        assert!(
...
#[test]
fn exact_snapshot_onboarding_generation_and_inspection_round_trip_through_binary() {
    ...
    let current = json_command(&["docs", "onboarding", "current", root_arg, "--json"]);
    assert_eq!(current["current"]["snapshot"], snapshot);
    assert_eq!(current["current"]["profile"], "onboarding");
    assert_eq!(current["current"]["generation"], generated["generation"]);

    let manifest = json_command(&["docs", "onboarding", "manifest", root_arg, "--json"]);
    assert_eq!(manifest["manifest"]["profile"], "onboarding");
    assert!(
        manifest["manifest"]["documents"]
            .as_array()
            .unwrap()
            .iter()
            .any(|document| document["path"] == "onboarding/index.md")
    );

    let validation = json_command(&["docs", "onboarding", "validation", root_arg, "--json"]);
    assert_eq!(validation["report"]["snapshot"], snapshot);
    assert_eq!(validation["report"]["profile"], "onboarding");
...
#[test]
fn missing_exact_snapshot_fails_without_onboarding_publication() {
...
fn fixture_root(label: &str) -> PathBuf {
    ...
    let root = std::env::temp_dir().join(format!(
        "athanor-onboarding-cli-{label}-{}-{nonce}",
        std::process::id()
    ));
    ...
    .expect("write onboarding fixture");
    root
}
