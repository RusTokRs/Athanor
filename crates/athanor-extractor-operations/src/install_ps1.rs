...
use athanor_domain::{Entity, EntityId, EntityKind, Fact, FactId, FactKind, LanguageCode, SourceLocation, StableKey};
...
pub(super) fn extract_install_ps1(
    ...
    let stable_key = StableKey(format!("script-command://{}#installer-powershell", input.source.path));
    ...
    let mut evidence_lines = vec![1, metadata.manifest_line, metadata.expected_files_line, metadata.checksum_line];
    evidence_lines.sort_unstable();
    evidence_lines.dedup();

    facts.push(Fact {
        id: FactId(format!("fact_script_command_defined_{:016x}", stable_hash(stable_key.0.as_bytes()))),
        ...
        evidence: evidence_lines.into_iter().map(|line|
            evidence_for_file(&input.source.path, extractor, Some(line), Some(line))
        ).collect(),
        ownership,
...
fn parse_install_ps1(content: &str) -> Option<InstallPs1Metadata> {
    ...
    for (index, raw_line) in content.lines().enumerate() {
        ...
        match raw_line.trim() {
            ...
            "$ActualHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $SourcePath).Hash.ToLowerInvariant()" => checksum_line = Some(line),
            _ => {}
        }
    }
...
mod tests {
    ...
    async fn projects_bounded_installer_and_checksum_metadata() {
        let output = OperationsExtractor.extract(ExtractInput {
