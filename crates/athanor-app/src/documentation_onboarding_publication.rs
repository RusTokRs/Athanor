//! Atomic publication for deterministic onboarding documentation.
//!
...
use crate::{
    ...
    DOCUMENTATION_DRAFT_SCHEMA_V1, DOCUMENTATION_MANIFEST_PATH,
...
const ONBOARDING_VALIDATION_REPORT_ID: &str = "onboarding-validation-report";
const JSON_MEDIA_TYPE: &str = "application/json";

#[derive(Debug, Clone)]
pub struct DocumentationOnboardingPublicationOptions {
    pub root: PathBuf,
    pub force: bool,
}
...
pub enum DocumentationOnboardingPublicationStatus {
    Published,
    UpToDate,
}

#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub struct DocumentationOnboardingPublicationReport {
    pub status: DocumentationOnboardingPublicationStatus,
    pub root: PathBuf,
...
pub fn publish_documentation_onboarding_generation(
    options: DocumentationOnboardingPublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
) -> Result<DocumentationOnboardingPublicationReport> {
    publish_documentation_onboarding_generation_inner(options, request, snapshot, None)
}

pub fn publish_documentation_onboarding_generation_cancellable(
    options: DocumentationOnboardingPublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
...
fn publish_documentation_onboarding_generation_inner(
    ...
    if request.profile != DocumentationProfile::Onboarding {
        anyhow::bail!("onboarding documentation publication requires profile `onboarding`");
    }
    ...
    let profile = build_documentation_onboarding_profile(request, snapshot)
        .map_err(anyhow::Error::msg)
        .context("failed to build deterministic onboarding profile")?;
    ...
    let manifest = DocumentationGenerationManifest {
        ...
        documents: vec![
            DocumentationDocumentManifest {
                id: ONBOARDING_DOCUMENT_ID.to_string(),
            ...
            DocumentationDocumentManifest {
                id: ONBOARDING_VALIDATION_REPORT_ID.to_string(),
...
fn load_current_generation(
    ...
    if current.schema != DOCUMENTATION_CURRENT_SCHEMA_V1
        ...
        || current.profile != DocumentationProfile::Onboarding
    ...
    if report.schema != DOCUMENTATION_VALIDATION_REPORT_SCHEMA_V1
        ...
        || report.profile != DocumentationProfile::Onboarding
...
fn manifest_matches_expected_artifacts(
    ...
    if manifest.documents.len() != 2 || manifest.profile != DocumentationProfile::Onboarding {
        return false;
    }
    let document = manifest
        .documents
        .iter()
        .find(|document| document.id == ONBOARDING_DOCUMENT_ID);
    let validation = manifest
        .documents
        .iter()
        .find(|document| document.id == ONBOARDING_VALIDATION_REPORT_ID);
    matches!(
        (document, validation),
        (Some(document), Some(validation))
            if document.path == ONBOARDING_DOCUMENT_PATH
                && document.media_type == ONBOARDING_DOCUMENT_MEDIA_TYPE
...
fn report_for_current(
    status: DocumentationOnboardingPublicationStatus,
    root: PathBuf,
    current: CurrentDocumentationGeneration,
    current_pointer: PathBuf,
    documentation_root: &Path,
) -> DocumentationOnboardingPublicationReport {
    let generation_dir = documentation_root.join(&current.path);
    DocumentationOnboardingPublicationReport {
        status,
        root,
        ...
        document: generation_dir.join(ONBOARDING_DOCUMENT_PATH),
