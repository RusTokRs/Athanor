//! Validated inspection of the current operations documentation generation.
...
use crate::{
    ...
    DocumentationValidationReport, DocumentationValidationStatus, OPERATIONS_DOCUMENT_MEDIA_TYPE,
...
pub struct DocumentationOperationsCurrentInspection {
    pub root: PathBuf,
...
#[derive(Debug, Clone, Serialize)]
pub struct DocumentationOperationsManifestInspection {
    pub root: PathBuf,
...
#[derive(Debug, Clone, Serialize)]
pub struct DocumentationOperationsValidationInspection {
    pub root: PathBuf,
...
pub fn inspect_documentation_operations_current(
    root: impl AsRef<Path>,
) -> Result<DocumentationOperationsCurrentInspection> {
    let resolved = resolve_current(root.as_ref())?;
    Ok(DocumentationOperationsCurrentInspection {
        root: resolved.root,
...
pub fn inspect_documentation_operations_manifest(
    root: impl AsRef<Path>,
) -> Result<DocumentationOperationsManifestInspection> {
    let resolved = resolve_current(root.as_ref())?;
    let manifest = load_validated_manifest(&resolved)?;
    Ok(DocumentationOperationsManifestInspection {
        root: resolved.root,
...
pub fn inspect_documentation_operations_validation(
    root: impl AsRef<Path>,
) -> Result<DocumentationOperationsValidationInspection> {
    let resolved = resolve_current(root.as_ref())?;
    let manifest = load_validated_manifest(&resolved)?;
...
fn load_validated_manifest(resolved: &ResolvedCurrent) -> Result<DocumentationGenerationManifest> {
    ...
    if manifest.generation != resolved.current.generation
        ...
        || manifest.profile != DocumentationProfile::Operations
    ...
    let document = manifest
        ...
        .context("documentation manifest omits operations Markdown")?;
    ...
    {
        bail!("documentation manifest contains an unsupported operations artifact layout");
    }
