//! Atomic publication for deterministic module documentation.
//!
...
use crate::{
    ...
    MODULE_DOCUMENT_MEDIA_TYPE, MODULE_DOCUMENT_PATH, build_documentation_module_profile,
};
...
#[derive(Debug, Clone)]
pub struct DocumentationModulePublicationOptions {
    pub root: PathBuf,
    pub force: bool,
}
...
pub enum DocumentationModulePublicationStatus {
    Published,
    UpToDate,
}

#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub struct DocumentationModulePublicationReport {
    pub status: DocumentationModulePublicationStatus,
    pub root: PathBuf,
...
/// Publishes a module document without cooperative cancellation.
pub fn publish_documentation_module_generation(
    options: DocumentationModulePublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
) -> Result<DocumentationModulePublicationReport> {
    publish_documentation_module_generation_inner(options, request, snapshot, None)
}

/// Publishes a module document while preserving the previous pointer on cancellation.
pub fn publish_documentation_module_generation_cancellable(
    options: DocumentationModulePublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
    cancellation: CancellationToken,
) -> Result<DocumentationModulePublicationReport> {
    publish_documentation_module_generation_inner(options, request, snapshot, Some(cancellation))
}

fn publish_documentation_module_generation_inner(
    options: DocumentationModulePublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
    ...
    if request.profile != DocumentationProfile::Module {
        anyhow::bail!("module documentation publication requires profile `module`");
    }
    ...
    let profile = build_documentation_module_profile(request, snapshot)
        .map_err(anyhow::Error::msg)
        .context("failed to build deterministic module profile")?;
...
fn load_current_generation(
    ...
    if current.schema != DOCUMENTATION_CURRENT_SCHEMA_V1
        ...
        || current.profile != DocumentationProfile::Module
...
fn manifest_matches_expected_artifacts(
    ...
    if manifest.documents.len() != 2 || manifest.profile != DocumentationProfile::Module {
        return false;
    }
...
fn report_for_current(
    status: DocumentationModulePublicationStatus,
    root: PathBuf,
    current: CurrentDocumentationGeneration,
    current_pointer: PathBuf,
    documentation_root: &Path,
) -> DocumentationModulePublicationReport {
    let generation_dir = documentation_root.join(&current.path);
    DocumentationModulePublicationReport {
        status,
        root,
        ...
        document: generation_dir.join(MODULE_DOCUMENT_PATH),
