...
use athanor_core::{
    AtomicSnapshotPublication, CanonicalSnapshotStore, CoreError, OperationContext, PreparedSnapshot,
...
/// Stages application artefacts behind a durable journal, then atomically publishes the complete
/// canonical batch and commit marker. The prepared handle remains the recovery/abort authority for
/// compatibility with journal v1/v2 and existing fault fixtures.
pub(crate) async fn publish_prepared_index(
    ...
    if let Err(publish_error) = store
        .publish_snapshot_batch_with_context(output.snapshot.clone(), batch, operation)
        .await
    {
        ...
        match exact_snapshot_is_committed(store, prepared.snapshot()).await {
            Ok(true) => {
...
/// Recovers one interrupted atomic or compatibility publication using the journal snapshot itself,
/// never only `LatestCommitted`, as the canonical commit authority.
...
async fn abort_prepared_with_error<T>(
    store: &AthanorStore,
    prepared: &PreparedSnapshot,
    error: anyhow::Error,
) -> Result<T> {
