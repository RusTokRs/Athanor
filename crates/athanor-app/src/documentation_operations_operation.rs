//! Composition-aware deterministic operations documentation generation.
//!
//! This owner resolves one project, initializes its configured canonical Store, loads the exact
//! committed snapshot named by the request, and delegates immutable operations publication.
...
use crate::{
    CancellationToken, DocumentationGenerationRequest, DocumentationOperationsPublicationOptions,
    DocumentationOperationsPublicationReport, DocumentationProfile, RuntimeComposition,
    publish_documentation_operations_generation,
    publish_documentation_operations_generation_cancellable,
};

#[derive(Debug, Clone)]
pub struct DocumentationOperationsOperationOptions {
    pub root: PathBuf,
    pub request: DocumentationGenerationRequest,
    pub force: bool,
}

pub async fn generate_documentation_operations_with_composition(
    options: DocumentationOperationsOperationOptions,
    composition: &RuntimeComposition,
) -> Result<DocumentationOperationsPublicationReport> {
    generate_documentation_operations_with_composition_inner(options, composition, None).await
}

pub async fn generate_documentation_operations_with_composition_cancellable(
    options: DocumentationOperationsOperationOptions,
    composition: &RuntimeComposition,
...
async fn generate_documentation_operations_with_composition_inner(
    ...
    if options.request.profile != DocumentationProfile::Operations {
        bail!("operations documentation operation requires profile `operations`");
    }
    ...
    let publication = DocumentationOperationsPublicationOptions {
        root,
        force: options.force,
    };
    match cancellation {
        Some(cancellation) => publish_documentation_operations_generation_cancellable(
            publication,
            &options.request,
            &snapshot,
            cancellation,
