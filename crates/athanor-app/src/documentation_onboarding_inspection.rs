//! Validated inspection of the current onboarding documentation generation.
...
use crate::{
    ...
    DocumentationValidationReport, DocumentationValidationStatus, ONBOARDING_DOCUMENT_MEDIA_TYPE,
    ONBOARDING_DOCUMENT_PATH,
};
...
pub struct DocumentationOnboardingCurrentInspection {
    pub root: PathBuf,
...
#[derive(Debug, Clone, Serialize)]
pub struct DocumentationOnboardingManifestInspection {
    pub root: PathBuf,
...
#[derive(Debug, Clone, Serialize)]
pub struct DocumentationOnboardingValidationInspection {
    pub root: PathBuf,
...
pub fn inspect_documentation_onboarding_current(
    root: impl AsRef<Path>,
) -> Result<DocumentationOnboardingCurrentInspection> {
    let resolved = resolve_current(root.as_ref())?;
    Ok(DocumentationOnboardingCurrentInspection {
        root: resolved.root,
...
pub fn inspect_documentation_onboarding_manifest(
    root: impl AsRef<Path>,
) -> Result<DocumentationOnboardingManifestInspection> {
    let resolved = resolve_current(root.as_ref())?;
    let manifest = load_validated_manifest(&resolved)?;
    Ok(DocumentationOnboardingManifestInspection {
        root: resolved.root,
...
pub fn inspect_documentation_onboarding_validation(
    root: impl AsRef<Path>,
) -> Result<DocumentationOnboardingValidationInspection> {
    let resolved = resolve_current(root.as_ref())?;
    let manifest = load_validated_manifest(&resolved)?;
...
fn resolve_current(root: &Path) -> Result<ResolvedCurrent> {
    ...
    if current.profile != DocumentationProfile::Onboarding {
        bail!("current documentation generation is not an onboarding profile");
    }
...
fn load_validated_manifest(resolved: &ResolvedCurrent) -> Result<DocumentationGenerationManifest> {
    ...
    if manifest.generation != resolved.current.generation
        ...
        || manifest.profile != DocumentationProfile::Onboarding
    ...
    let document = manifest
        ...
        .context("documentation manifest omits onboarding Markdown")?;
    ...
    {
        bail!("documentation manifest contains an unsupported onboarding artifact layout");
    }
