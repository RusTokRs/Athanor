//! Atomic publication for deterministic API documentation.
//!
//! This owner publishes a supplied canonical snapshot into the shared immutable documentation
//! generation root. It deliberately does not load a Store or expose a transport command.
...
use crate::{
    API_DOCUMENT_MEDIA_TYPE, API_DOCUMENT_PATH, CancellationToken, CurrentDocumentationGeneration,
...
const API_DOCUMENT_ID: &str = "api-overview";
const API_VALIDATION_REPORT_ID: &str = "api-validation-report";
const JSON_MEDIA_TYPE: &str = "application/json";

#[derive(Debug, Clone)]
pub struct DocumentationApiPublicationOptions {
    pub root: PathBuf,
    pub force: bool,
}
...
pub enum DocumentationApiPublicationStatus {
    Published,
    UpToDate,
}

#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub struct DocumentationApiPublicationReport {
    pub status: DocumentationApiPublicationStatus,
    pub root: PathBuf,
...
/// Publishes an API document without cooperative cancellation.
pub fn publish_documentation_api_generation(
    options: DocumentationApiPublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
) -> Result<DocumentationApiPublicationReport> {
    publish_documentation_api_generation_inner(options, request, snapshot, None)
}

/// Publishes an API document while preserving the previous pointer on cancellation.
pub fn publish_documentation_api_generation_cancellable(
    options: DocumentationApiPublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
    cancellation: CancellationToken,
) -> Result<DocumentationApiPublicationReport> {
    publish_documentation_api_generation_inner(options, request, snapshot, Some(cancellation))
}

fn publish_documentation_api_generation_inner(
    options: DocumentationApiPublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
    ...
    if request.profile != DocumentationProfile::Api {
        anyhow::bail!("API documentation publication requires profile `api`");
    }
    ...
    let profile = build_documentation_api_profile(request, snapshot)
        .map_err(anyhow::Error::msg)
        .context("failed to build deterministic API profile")?;
        ...
        )?
    {
        return Ok(report_for_current(
            DocumentationApiPublicationStatus::UpToDate,
            root,
            current,
            current_pointer,
            &documentation_root,
    ...
    write_output_file(&staging.join(API_DOCUMENT_PATH), &profile.document.content)
        .context("failed to write API Markdown")?;
    write_output_file(
...
fn load_current_generation(
    ...
    if current.schema != DOCUMENTATION_CURRENT_SCHEMA_V1
        ...
        || current.profile != DocumentationProfile::Api
    ...
    if report.schema != DOCUMENTATION_VALIDATION_REPORT_SCHEMA_V1
        ...
        || report.profile != DocumentationProfile::Api
...
fn manifest_matches_expected_artifacts(
    ...
    if manifest.documents.len() != 2 || manifest.profile != DocumentationProfile::Api {
        return false;
    }
    let document = manifest
        .documents
        .iter()
        .find(|document| document.id == API_DOCUMENT_ID);
    let validation = manifest
        .documents
        .iter()
        .find(|document| document.id == API_VALIDATION_REPORT_ID);
    matches!(
        (document, validation),
        (Some(document), Some(validation))
            if document.path == API_DOCUMENT_PATH
                && document.media_type == API_DOCUMENT_MEDIA_TYPE
...
fn report_for_current(
    status: DocumentationApiPublicationStatus,
    root: PathBuf,
    current: CurrentDocumentationGeneration,
    current_pointer: PathBuf,
    documentation_root: &Path,
) -> DocumentationApiPublicationReport {
    let generation_dir = documentation_root.join(&current.path);
    DocumentationApiPublicationReport {
        status,
        root,
        ...
        document: generation_dir.join(API_DOCUMENT_PATH),
