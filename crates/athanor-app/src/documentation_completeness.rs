...
impl DocumentationCompletenessRequest {
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.snapshot.trim().is_empty() || self.snapshot.trim() != self.snapshot {
            return Err("documentation completeness snapshot must be non-empty and trimmed".to_string());
        }
        if self.limit == 0 {
...
fn collect_fact(
    ...
    let locations = evidence_locations(&fact.evidence, fact.ownership.iter().map(|owner| &owner.source_file));
    let paths = locations
        .into_iter()
        .map(|location| location.path)
        .collect::<BTreeSet<_>>();
