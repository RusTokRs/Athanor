//! Atomic publication for deterministic operations documentation.
//!
...
const OPERATIONS_VALIDATION_REPORT_ID: &str = "operations-validation-report";
const JSON_MEDIA_TYPE: &str = "application/json";

#[derive(Debug, Clone)]
pub struct DocumentationOperationsPublicationOptions {
    pub root: PathBuf,
    pub force: bool,
}
...
pub enum DocumentationOperationsPublicationStatus {
    Published,
    UpToDate,
}

#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub struct DocumentationOperationsPublicationReport {
    pub status: DocumentationOperationsPublicationStatus,
    pub root: PathBuf,
...
pub fn publish_documentation_operations_generation(
    options: DocumentationOperationsPublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
) -> Result<DocumentationOperationsPublicationReport> {
    publish_documentation_operations_generation_inner(options, request, snapshot, None)
}

pub fn publish_documentation_operations_generation_cancellable(
    options: DocumentationOperationsPublicationOptions,
    request: &DocumentationGenerationRequest,
    snapshot: &CanonicalSnapshot,
...
fn publish_documentation_operations_generation_inner(
    ...
    if request.profile != DocumentationProfile::Operations {
        anyhow::bail!("operations documentation publication requires profile `operations`");
    }
    ...
    let profile = build_documentation_operations_profile(request, snapshot)
        .map_err(anyhow::Error::msg)
        .context("failed to build deterministic operations profile")?;
    ...
    let manifest = DocumentationGenerationManifest {
        ...
        documents: vec![
            DocumentationDocumentManifest {
                id: OPERATIONS_DOCUMENT_ID.to_string(),
            ...
            DocumentationDocumentManifest {
                id: OPERATIONS_VALIDATION_REPORT_ID.to_string(),
...
fn load_current_generation(
    ...
    if current.schema != DOCUMENTATION_CURRENT_SCHEMA_V1
        ...
        || current.profile != DocumentationProfile::Operations
    ...
    if report.schema != DOCUMENTATION_VALIDATION_REPORT_SCHEMA_V1
        ...
        || report.profile != DocumentationProfile::Operations
...
fn manifest_matches_expected_artifacts(
    ...
    if manifest.documents.len() != 2 || manifest.profile != DocumentationProfile::Operations {
        return false;
    }
    let document = manifest
        .documents
        .iter()
        .find(|document| document.id == OPERATIONS_DOCUMENT_ID);
    let validation = manifest
        .documents
        .iter()
        .find(|document| document.id == OPERATIONS_VALIDATION_REPORT_ID);
    matches!(
        (document, validation),
        (Some(document), Some(validation))
            if document.path == OPERATIONS_DOCUMENT_PATH
                && document.media_type == OPERATIONS_DOCUMENT_MEDIA_TYPE
...
fn report_for_current(
    status: DocumentationOperationsPublicationStatus,
    root: PathBuf,
    current: CurrentDocumentationGeneration,
    current_pointer: PathBuf,
    documentation_root: &Path,
) -> DocumentationOperationsPublicationReport {
    let generation_dir = documentation_root.join(&current.path);
    DocumentationOperationsPublicationReport {
        status,
        root,
        ...
        document: generation_dir.join(OPERATIONS_DOCUMENT_PATH),
