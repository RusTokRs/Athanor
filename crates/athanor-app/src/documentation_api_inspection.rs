//! Validated inspection of the current API documentation generation.
...
use crate::{
    API_DOCUMENT_MEDIA_TYPE, API_DOCUMENT_PATH, CurrentDocumentationGeneration,
...
pub struct DocumentationApiCurrentInspection {
    pub root: PathBuf,
...
#[derive(Debug, Clone, Serialize)]
pub struct DocumentationApiManifestInspection {
    pub root: PathBuf,
...
#[derive(Debug, Clone, Serialize)]
pub struct DocumentationApiValidationInspection {
    pub root: PathBuf,
...
pub fn inspect_documentation_api_current(
    root: impl AsRef<Path>,
) -> Result<DocumentationApiCurrentInspection> {
    let resolved = resolve_current(root.as_ref())?;
    Ok(DocumentationApiCurrentInspection {
        root: resolved.root,
...
pub fn inspect_documentation_api_manifest(
    root: impl AsRef<Path>,
) -> Result<DocumentationApiManifestInspection> {
    let resolved = resolve_current(root.as_ref())?;
    let manifest = load_validated_manifest(&resolved)?;
    Ok(DocumentationApiManifestInspection {
        root: resolved.root,
...
pub fn inspect_documentation_api_validation(
    root: impl AsRef<Path>,
) -> Result<DocumentationApiValidationInspection> {
    let resolved = resolve_current(root.as_ref())?;
    let manifest = load_validated_manifest(&resolved)?;
...
fn resolve_current(root: &Path) -> Result<ResolvedCurrent> {
    ...
    if current.profile != DocumentationProfile::Api {
        bail!("current documentation generation is not an API profile");
    }
...
fn load_validated_manifest(resolved: &ResolvedCurrent) -> Result<DocumentationGenerationManifest> {
    ...
    if manifest.generation != resolved.current.generation
        ...
        || manifest.profile != DocumentationProfile::Api
    ...
    let document = manifest
        ...
        .context("documentation manifest omits API Markdown")?;
    ...
    {
        bail!("documentation manifest contains an unsupported API artifact layout");
    }
