use athanor_app::{
    ...
    DocumentationGenerationManifest, DocumentationGenerationRequest, VERSIONED_JSON_CONTRACTS,
};
...
#[test]
fn schema_drift_unknown_fields_and_unsafe_outputs_fail_closed() {
    let fixture: Value = serde_json::from_str(FIXTURE).expect("valid documentation fixture");
    ...
    let mut unsafe_manifest: DocumentationGenerationManifest =
        serde_json::from_value(fixture["manifest"].clone()).unwrap();
    unsafe_manifest.documents[0].path = "../editable.md".to_string();
    assert!(unsafe_manifest.validate_for_request(&request).is_err());
