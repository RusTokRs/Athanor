...
const MCP_TOOLS_SOURCE: &str = include_str!("../../athanor-transport-mcp/src/tools.rs");
...
fn mcp_runtime_composition_is_explicit_end_to_end() {
    ...
    assert!(MCP_TOOLS_SOURCE.contains("index_project_with_composition"));
    assert!(MCP_TOOLS_SOURCE.contains("check_project_with_composition"));
    assert!(!MCP_TOOLS_SOURCE.contains("athanor_app::index_project("));
    assert!(!MCP_TOOLS_SOURCE.contains("athanor_app::search_project("));
}
