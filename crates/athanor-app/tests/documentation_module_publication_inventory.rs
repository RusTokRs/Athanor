...
use athanor_app::{
    ...
    DocumentationModulePublicationOptions, DocumentationModulePublicationStatus,
    DocumentationProfile, DocumentationValidationReport, MODULE_DOCUMENT_PATH,
...
#[test]
fn module_publication_is_immutable_checksum_bound_and_reuses_exact_current() {
    ...
    assert_eq!(current.profile, DocumentationProfile::Module);
    ...
    manifest
        ...
        .expect("published manifest matches module request");
    assert_eq!(manifest.documents.len(), 2);
    assert!(
        manifest
            .documents
            .iter()
            .any(|document| document.path == MODULE_DOCUMENT_PATH)
    );
    ...
    assert_eq!(validation.profile, DocumentationProfile::Module);
...
#[test]
fn incomplete_or_tampered_module_generation_is_never_reused() {
    ...
    let repaired_manifest =
        publish_documentation_module_generation(options(&project, false), &request, &snapshot)
            .expect("incomplete module manifest must create a new generation");
    assert_eq!(
    ...
    let repaired_document =
        publish_documentation_module_generation(options(&project, false), &request, &snapshot)
            .expect("module checksum mismatch must create a new generation");
...
#[test]
fn cancellation_preserves_existing_module_pointer_and_generation_set() {
    ...
    cancellation.cancel();
    let error = publish_documentation_module_generation_cancellable(
        options(&project, true),
        &request,
        &snapshot,
        cancellation,
    )
    .expect_err("cancelled module publication must fail");
...
#[test]
fn module_publication_content_is_deterministic_across_canonical_input_order() {
...
fn fixture_snapshot() -> CanonicalSnapshot {
    serde_json::from_str(FIXTURE).expect("valid canonical module fixture")
}
...
fn options(project: &TempProject, force: bool) -> DocumentationModulePublicationOptions {
...
impl TempProject {
    fn new(label: &str) -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "athanor-module-publication-{label}-{}-{id}",
            std::process::id()
        ));
