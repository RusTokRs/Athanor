...
use athanor_app::{
    API_DOCUMENT_PATH, CancellationToken, CurrentDocumentationGeneration,
    DOCUMENTATION_CURRENT_SCHEMA_V1, DOCUMENTATION_MANIFEST_PATH,
    DOCUMENTATION_VALIDATION_REPORT_PATH, DocumentationApiPublicationOptions,
    DocumentationApiPublicationStatus, DocumentationGenerationLimits,
    DocumentationGenerationManifest, DocumentationGenerationRequest, DocumentationProfile,
    DocumentationValidationReport, publish_documentation_api_generation,
...
#[test]
fn api_publication_is_immutable_checksum_bound_and_reuses_exact_current() {
    ...
    let first = publish_documentation_api_generation(options(&project, false), &request, &snapshot)
        .expect("publish first API documentation generation");
    assert_eq!(first.status, DocumentationApiPublicationStatus::Published);
    ...
    assert_eq!(current.profile, DocumentationProfile::Api);
    ...
    manifest
        ...
        .expect("published manifest matches API request");
    assert_eq!(manifest.documents.len(), 2);
    assert!(
        manifest
            .documents
            .iter()
            .any(|document| document.path == API_DOCUMENT_PATH)
    );
...
#[test]
fn incomplete_or_tampered_api_generation_is_never_reused() {
    ...
    let repaired_manifest =
        publish_documentation_api_generation(options(&project, false), &request, &snapshot)
            .expect("incomplete API manifest must create a new generation");
    assert_eq!(
    ...
    let repaired_document =
        publish_documentation_api_generation(options(&project, false), &request, &snapshot)
            .expect("API checksum mismatch must create a new generation");
    ...
    let forced = publish_documentation_api_generation(options(&project, true), &request, &snapshot)
        .expect("force must publish another immutable API generation");
    assert_eq!(forced.generation, "00000004");
    assert_eq!(forced.status, DocumentationApiPublicationStatus::Published);
    assert_eq!(generation_count(&project), 4);
    assert!(forced.generation_dir.join(API_DOCUMENT_PATH).is_file());
    assert!(
        forced
            .generation_dir
...
#[test]
fn cancellation_preserves_existing_api_pointer_and_generation_set() {
    ...
    cancellation.cancel();
    let error = publish_documentation_api_generation_cancellable(
        options(&project, true),
        &request,
        &snapshot,
        cancellation,
    )
    .expect_err("cancelled API publication must fail");
...
#[test]
fn api_publication_content_is_deterministic_across_canonical_input_order() {
...
fn snapshot() -> CanonicalSnapshot {
    CanonicalSnapshot {
        snapshot: Some(SnapshotId("snap-api-publication".to_string())),
        entities: vec![Entity {
...
fn options(project: &TempProject, force: bool) -> DocumentationApiPublicationOptions {
...
impl TempProject {
    fn new(label: &str) -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "athanor-api-publication-{label}-{}-{id}",
            std::process::id()
        ));
