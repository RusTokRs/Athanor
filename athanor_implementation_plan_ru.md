...
| `DS-JSON-003` | P1 | `[-] in progress` | Typed CLI/daemon/MCP payload parity реализована; executable parity verification pending |
...
- [ ] Выполнить targeted contract regressions.
- [ ] Добавить или выполнить executable CLI/daemon parity regressions для оставшихся общих operations.
- [ ] Выполнить workspace tests и Clippy.

### Definition of Done

`DS-JSON-001` станет `[x] verified`, когда targeted fixtures, executable parity, workspace tests, fmt и Clippy фактически пройдут. Implementation inventory в audited scope завершена.
...
- [ ] Выполнить executable CLI/daemon Index parity regression.
- [ ] Выполнить executable CLI/daemon Generation parity regression.
- [ ] Выполнить executable Context parity regression.
- [ ] Выполнить executable Wiki/HTML parity regressions.
- [ ] Распространить executable parity verification на remaining shared operations при необходимости.

## Оценка остатка

Implementation inventory завершена. Остался **1 verification-пакет**:
...
По объёму запланированной реализации осталось ориентировочно **0–3%**; фактический остаток зависит от того, выявит ли verification compile/fmt/Clippy regressions.
...
**Сейчас:** `DS-JSON-001/002/003` — implementation inventory завершена: 59 public/current owners, 30 non-public schemas, 4 MCP boundaries и 4 process protocols.

**Дальше:** полный verification-проход и только подтверждённые им исправления.
