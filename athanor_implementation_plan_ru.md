...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter contracts, explicit composition и первый publication-hardening slice реализованы; execution/cleanup pending |
...
| `TOOL-001` | P2 | `[!] blocked` | GitHub contents connector не поддерживает точечный patch больших Rust-файлов | Получена проверяемая полная копия blob либо доступен безопасный patch path |
...
- [ ] `PUB-003`: добавить failure injection для stage/publish/rollback/cleanup.
...
**Сейчас:** `PUB-002` находится `[-] in progress`. Общий staged directory/file replacement и daemon endpoint исправлены; полный durability inventory и source-regression находятся в `main`. Execution evidence отсутствует.

**Следом:** `PUB-004` — app-level finalizers/registry/journal; затем `PUB-005` — JSONL pointer writers через безопасный patch path; после этого `PUB-003` failure injection. `RUST-001`/`RUST-002` и `COMP-005` остаются заблокированы безопасной правкой большого `runtime.rs`.
