...
| 4.8 | частично выполнен | explicit composition вынесена в `composition.rs`, async `TokioProcessRunner` — в `runtime/process_runner.rs`, bounded execution/support внешних process-adapter — в `runtime/process_adapter.rs` и `runtime/process_adapter_support.rs`, plugin discovery/trust/hash/status — в focused `runtime/plugin_*.rs` modules, pipeline phases — в `pipeline_source.rs`, `pipeline_extract.rs`, `pipeline_link.rs`, `pipeline_check.rs`, а merge/metrics/ownership/support — в отдельные focused modules; daemon client transport — в `daemon_client.rs`, server connection I/O/busy rejection — в `daemon_connection.rs`, protocol responses/limits, request-shape/deadline и authentication-policy validation — в `daemon_protocol.rs`, endpoint I/O — в `daemon_endpoint.rs`, watcher/debounce — в `daemon_watcher.rs`, read-only job registry — в `daemon_job_registry.rs`, lifecycle/active-job state, shutdown cancellation и bounded drain — в `daemon_lifecycle.rs`, cancellation transitions — в `daemon_job_cancellation.rs`, queued-job allocation — в `daemon_job_scheduler.rs`, running/terminal job state — в `daemon_job_state.rs`, чистые job helpers — в `daemon_jobs_support.rs`, invalidation — в отдельный module, daemon query `BoundedCache` — в `daemon_runtime.rs`. `runtime.rs` и daemon command orchestration всё ещё требуют дальнейшей декомпозиции. |
| 4.9 | частично выполнен | core имеет stable `CoreErrorCode`, retryability и typed conflict/snapshot/busy/cancel/deadline categories, включая distinct `adapter_protocol` для malformed JSON output external adapter; daemon v3 публикует endpoint и structured `error_details` (code/retryable/message/details), сохраняя legacy строку `error` и v2 endpoint/request/response compatibility. Точное end-to-end mapping всех error categories и MCP migration не завершены. |
