...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP lifecycle/composition, adapter contracts, publication hardening, JSONL store и bounded CLI root реализованы; execution и remaining cleanup pending |
...
| `COMP-008` | P2 | `[-] in progress` | API snapshot/registry, Docs check/drift/propose/apply и Repair facades временно используют task-local store composition вокруг legacy service internals | Выполнить isolation regressions, внедрить прямые service composition APIs и удалить task-local bridges |
...
- [x] Index, Explain, Search, Context, Impact, Change Map, RusTok context и Check вызывают composition-aware application APIs.
- [x] Legacy `src/lib.rs`, `include!("lib.rs")` и MCP-local `athanor_runtime_defaults::install()` удалены.
...
- [x] Search, Context, Check, Graph, ValidateChanged, RusTok, Generation, Direct Read, Index/Bench, Docs, API, Analysis и MCP используют explicit/scoped composition.
...
- [x] Source regressions защищают active routing, facade composition, MCP composition и task-local isolation.
...
**Сейчас:** `MCP-005` и `COMP-006` отмечены `[x] implemented`: transport получает explicit `RuntimeComposition`, legacy tool dispatcher/include и MCP-local global bootstrap удалены. Execution evidence отсутствует.

**Следом:** `COMP-008` — заменить task-local API/Docs/Repair service bridges прямыми composition internals и выполнить isolation regressions. Параллельно остаются `MCP-004`, transactional Index cancellation review и Rust-enabled verification.
...
- Удалены `crates/athanor-transport-mcp/src/lib.rs`, textual `include!("lib.rs")` и MCP-local `athanor_runtime_defaults::install()`.
- Transport crate не зависит от runtime-defaults; production composition остаётся обязанностью CLI composition root.
- Unit protocol tests используют fail-closed test-only composition без реальных adapters/stores/projectors.
- `mcp_transport_contracts.rs` и `mcp_runtime_bootstrap_inventory.rs` проверяют explicit composition, отсутствие legacy calls и bounded modules.
...
- Для Docs/API registry и benchmark добавлены scoped/direct composition entrypoints.
- `CLI-001`, `CLI-005` и `TOOL-002` отмечены `[x] implemented`.
...
- Post-commit backup cleanup возвращает success; invalid non-file target fail-closed до commit.
...
- Добавлены composition-aware Graph, RusTok, API/Docs, benchmark, MCP и Repair paths.
