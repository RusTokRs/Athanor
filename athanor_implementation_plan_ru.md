...
| `DS-JSON-001` | P1 | `[-] in progress` | Versioned JSON registry, ownership checks и девять golden-protected v1 contracts в `main` |
...
- [x] Девять public report types реализуют общий contract trait.
- [x] `RepositoryOverview` и `EntityExplanation` создают schema через shared constants.
- [x] Локальные Coverage/Capabilities constants проверяются на равенство shared registry constants.
...
- [x] Fixtures фиксируют nested entities, enum scopes, counters, limits, omission blocks и nested report composition.
...
- [ ] Следующей группой рассмотреть ChangeMap, Graph, Context и Project Registry reports.
- [ ] Назначить одному Rust type единственного schema owner.
- [ ] Заменить оставшиеся локальные schema literals shared constants, включая Search, Impact и Check builders.
...
**Сейчас:** `DS-JSON-001` — девять first-wave contracts зарегистрированы и защищены fixtures; verification pending.

**Дальше:** remaining inventory, shared-literal migration и CLI/daemon/MCP parity tests.
