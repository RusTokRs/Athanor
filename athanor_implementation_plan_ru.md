...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter contracts, publication hardening, JSONL store и bounded CLI root реализованы; MCP composition, execution и remaining cleanup pending |
...
| `MCP-005` | P1 | `[-] in progress` | Transport runtime всё ещё включает legacy tool dispatcher, который вызывает global app APIs; после удаления старого CLI root bootstrap локализован в `mcp_cli.rs` | `RuntimeComposition` передаётся через server/request/tool lifecycle; legacy `include!("lib.rs")` и MCP-local global install удалены; transport regressions выполнены |
...
| `COMP-006` | P2 | `[-] in progress` | Focused handlers устанавливали process-global runtime перед explicit-composition calls | Все обычные CLI families используют explicit/scoped composition; остаётся только MCP compatibility bootstrap из `MCP-005` |
...
| `COMP-008` | P2 | `[-] in progress` | API snapshot/registry, Docs check/drift/propose/apply и Repair facades временно используют task-local store composition вокруг legacy service internals | CLI wrappers удалены; выполнить isolation regressions, внедрить прямые service composition APIs и удалить task-local bridges |
...
| `CLI-001` | P2 | `[x] implemented` | Корень CLI включал 4k-line `main.rs`, legacy fallback и дублирующую grammar/rendering | `main.rs` и последний include удалены; все команды принадлежат typed bounded root/family models; source regressions находятся в `main`; execution parity pending |
...
| `TOOL-002` | P2 | `[x] implemented` | Contents connector не позволял безопасно изменить 4k-line CLI monolith точечным patch | Ограничение обойдено полным переносом всех command families; `apps/ath/src/main.rs` удалён без compatibility shim |
...
- [x] После удаления legacy CLI root необходимый compatibility bootstrap локализован только в `mcp_cli.rs` и защищён inventory test.

Осталось:

- [ ] `MCP-005`: передать `RuntimeComposition` через server/request/tool lifecycle и удалить transport legacy include/bootstrap.
...
- [x] Search, Context, Check, Graph, ValidateChanged, RusTok, Generation, Direct Read, Index/Bench, Docs, API и Analysis используют explicit/scoped composition.
- [x] `entry.rs` не содержит `athanor_runtime_defaults::install()`.
- [x] Source regressions защищают active routing, facade composition и task-local isolation.

Осталось:

- [ ] Удалить MCP-local compatibility bootstrap (`MCP-005`).
- [ ] Выполнить source, targeted, isolation и executable regressions.
- [ ] Добавить executable regression для focused `validate-changed`.
- [ ] Заменить task-local compatibility service internals прямыми composition APIs (`COMP-008`).
...
- [x] Daemon endpoint использует общий atomic replacement вместо delete-before-rename.
- [x] Repair retention/recovery остаются intentionally strict destructive/recovery operations.
...
- [x] Repository-wide source regression запрещает старые propagating cleanup errors и duplicate JSONL writers.
...
- [x] Выделены `store`, `lifecycle`, `canonical`, `latest`, `pointer_publication`, `snapshot_io`, `indexes`, `commit_marker`, `atomic_publication`.
- [x] Все production-модули ограничены 300 строками source regression.
- [x] Public API `JsonlKnowledgeStore`/`PathIndex*` сохранён.
- [x] Strict latest, legacy query compatibility, repair normalization, sequence allocation и indexes покрыты regressions.
...
- [x] API snapshot/registry и Docs operations используют composition-aware facades; versioned JSON reports сохранены.
- [x] Direct Read разделён на `model`, `operation`, `run`, `render/entity`, `render/change`, `render/support`.
- [x] Repair разделён на `model`, `run`, `render`, parser tests.
- [x] Graph, Check и API diff используют отдельные renderer-модули без legacy namespace.
- [x] RusTok разделён на `model`, `run` и общий renderer boundary; старый 688-line command file удалён.
- [x] Source enforcement запрещает family include, namespace shadowing, root legacy fallback, renderer bridges и рост production modules выше заданных лимитов.

Осталось:

- [ ] Выполнить parser/help/version/unknown-command regressions.
- [ ] Выполнить JSON/text rendering и executable parity regressions для каждой family.
- [ ] Проверить MCP после полного `MCP-005` composition refactor.
...
**Сейчас:** `CLI-001` и `CLI-005` отмечены `[x] implemented`: монолитный `main.rs`, include и legacy fallback удалены; все команды принадлежат bounded typed families. Execution evidence отсутствует.

**Следом:** `MCP-005` — передать `RuntimeComposition` через MCP server/request/tool dispatcher и удалить локализованный global bootstrap. После этого продолжить `COMP-008`: заменить task-local API/Docs/Repair service bridges прямыми composition internals.

## Журнал

### 2026-07-17 — Complete CLI root migration

- Перенесены legacy-only Index/Bench/Update, Docs, API, Projects, Analysis и MCP grammars в bounded typed modules.
- `root_command.rs` теперь владеет всеми routes, root help/version и unknown-command behavior без legacy fallback.
- Удалены `apps/ath/src/main.rs`, последний `include!("main.rs")`, duplicate Application Report interceptor и duplicate Projection family.
- Wiki/HTML/Generate объединены под одним composition-aware Generation owner.
- Сохранены tracing, отдельный CLI thread и 8 MiB stack.
- Для Docs/API registry и benchmark добавлены scoped/direct composition entrypoints, найденные при удалении global root bootstrap.
- MCP legacy dependency локализована в dedicated process bootstrap и записана как `MCP-005`; source inventory запрещает скрывать её в entrypoint.
- `CLI-001`, `CLI-005` и `TOOL-002` отмечены `[x] implemented`; `COMP-006` возвращён в `[-] in progress` до закрытия MCP-005.
- Rust execution evidence отсутствует.
...
- RusTok разделён на model/run/render; старый monolithic command file удалён.
- Добавлен typed `root_command::Command`, который единолично задаёт порядок focused parsers.
- Из `entry.rs` удалены все renderer bridge functions.
- `legacy_factory_migration.rs` проверяет root routes, отсутствие legacy imports и bounded modules.
- `CLI-004` отмечен `[x] implemented`; Rust execution evidence отсутствует.

### 2026-07-17 — Focused CLI family modularization

- Direct Read разбит на model/operation/run/render modules; deadline и cancellation regressions сохранены.
- Application Report напрямую вызывает composition-aware snapshot/docs facades.
- Repair разбит на model/run/render с сохранением manual parser и help parity.
- Удалены composition/include wrappers и monolithic command bodies общей длиной более 1100 строк.
- `CLI-003` отмечен `[x] implemented`; Rust execution evidence отсутствует.

### 2026-07-17 — JSONL store module and pointer publication refactor

- `athanor-store-jsonl` переведён с `strict_latest -> include!(lib) -> include!(store)` на conventional module tree.
- Старые `lib.rs` 578 строк, `store.rs` 1286 строк и wrapper удалены/разделены на модули по 98–273 строки.
- Strict latest и snapshot sequence используют один `pointer_publication` helper.
- Post-commit backup cleanup возвращает success и оставляет recovery-visible backup; invalid non-file target fail-closed до commit.
- Добавлены pointer fault test, module-boundary/line-limit regression и behavioral refactor regression.
- `PUB-002`, `PUB-005`, `STORE-001` отмечены `[x] implemented`; Rust execution evidence отсутствует.

### 2026-07-17 — App publication cleanup slice

- Project registry, publication journal, index-state и read-model cleanup после commit point переведены на warning-path.
- Source regression защищает отсутствие propagating maintenance errors.
- `PUB-004` отмечен `[x] implemented`; Rust execution evidence отсутствует.

### 2026-07-17 — Publication failure matrix

- Добавлены stage/build, rollback restoration, post-commit cleanup, Drop, target-race и repair recovery regressions.
- `PUB-003` отмечен `[x] implemented`; execution evidence отсутствует.

### 2026-07-17 — Explicit-composition migration

- Active focused handlers переведены на explicit/scoped composition без entry-level global install.
- Добавлены composition-aware Graph, RusTok, API/Docs, benchmark и Repair paths.
- `COMP-008` остаётся in progress до удаления task-local service bridges.
