...
**Сейчас:** `COMP-006` реализован в `main`: active focused handlers не устанавливают process-global runtime. Application Report и Repair используют task-local explicit store composition как compatibility bridge; isolation regression находится в коде. Execution evidence отсутствует.
