...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter contracts и четыре explicit-composition slice реализованы; execution/cleanup pending |
...
| `COMP-006` | P2 | `[-] in progress` | Focused handlers создают composition, но часть всё ещё вызывает legacy `athanor_runtime_defaults::install()` | Search/Context/Check/Graph/ValidateChanged/RusTok очищены; остальные handlers migrated/classified |
...
- [x] Focused Search, Context, Check, Graph, ValidateChanged и RusTok не вызывают global `install()`.
...
- [ ] Выполнить Rustok/source и executable regressions.
...
**Сейчас:** `COMP-007` реализован в `main`: RusTok context/audit/graph family использует explicit `RuntimeComposition`; active focused CLI не устанавливает process-global runtime. Execution evidence отсутствует.

**Следом:** закрыть оставшуюся часть `COMP-006` — классифицировать и перевести Direct Read/Application Report/Generation и другие focused handlers; затем безопасный repair `runtime.rs`, legacy trust API cleanup и publication inventory.
