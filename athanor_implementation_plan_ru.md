...
| `DS-JSON-001` | P1 | `[-] in progress` | Registry содержит 31 golden-protected public contract; app-layer migration allowlist пуст |
| `DS-JSON-002` | P1 | `[-] in progress` | App-layer inventory завершена; transport/process/persistence inventory остаётся |
...
- [x] Отдельный macro для transparent wrapper contracts использует явный `AsRef`, а не неявный field autoderef.
...
- [x] Всего 31 public report type реализует общий contract trait.
...
- [x] Coverage/Capabilities, core Graph, RusTok, Project Registry и Project Resolution constants проверяются на равенство shared registry constants.
...
- [x] Известных app-layer schema collisions и public migration exceptions не осталось.
...
- [x] Завершить app-layer public contract inventory.
- [x] Закрыть Context и Project Registry schema collisions.
- [x] Зарегистрировать FFA/FBA/Page Builder specialized families.
...
`DS-JSON-001` станет `[x] verified`, когда transport/process/persistence inventory завершена, parity regressions пройдены, fixtures выполняются, а fmt, targeted tests, workspace tests и Clippy зелёные.
...
- [x] Зафиксирован 31 зарегистрированный public contract и его Rust owner.
- [x] App-layer scanner охватывает известные public owner modules.
- [x] App-layer public migration allowlist пуст.
- [x] Новые app-layer schema literals должны быть зарегистрированы или явно classified.
...
- [x] Operation-aware Rustok graph outputs используют command-specific transparent contracts.
...
**Сейчас:** `DS-JSON-001/002/003` — 31 public contract зарегистрирован; app-layer public inventory и specialized families закрыты на уровне реализации; verification остаётся pending.

**Дальше:** daemon/MCP envelopes, process-adapter protocols и persisted/generated document inventory, затем executable parity enforcement.

## Журнал

### 2026-07-16 — DS-JSON-001/002 specialized RusTok families complete
...
- Добавлены representative non-empty fixtures для всех трёх families.
- Specialized schema ids удалены из migration allowlist; app-layer allowlist стал пустым.
- Registry вырос с 19 до 31 public contracts.
- `json_contract.rs` возвращён к обычному formatting и использует explicit transparent-owner macro.
