...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP lifecycle/composition, adapter contracts, publication hardening, JSONL store, bounded CLI root и direct Docs composition реализованы; execution и remaining cleanup pending |
...
| `COMP-008` | P2 | `[x] implemented` | API/Docs/Repair facades использовали task-local store composition вокруг legacy service internals | API snapshot/registry, Repair recovery/latest и Docs check/drift/propose/apply используют прямой `composition.init_store`; последний task-local bridge удалён; source/isolation regressions в `main`; execution pending |
...
| `DOC-003` | P2 | `[ ] planned` | Docs contract и proposal engine всё ещё размещены в 3k-line compatibility owner через `include!("docs/legacy_impl.rs")` | Разделить shared docs builders/helpers на conventional bounded modules и удалить textual include без изменения public API/JSON behavior |
...
- [x] Docs check/drift/propose/apply разделены на bounded `docs_direct/{snapshot,check,propose,apply}.rs` и вызывают `composition.init_store` напрямую.
...
- [x] Source regressions запрещают `with_store_composition`/global Store lookup в API/Docs/Repair direct paths и фиксируют module bounds.
...
- [ ] Удалить compatibility include после декомпозиции Docs engine (`DOC-003`).
...
- [ ] `DOC-003`: разделить compatibility Docs owner и удалить textual include.
...
**Сейчас:** `COMP-008` закрыт как `[x] implemented`: API snapshot/registry, Repair recovery/latest и Docs check/drift/propose/apply используют direct composition; task-local Store service bridges в этих paths отсутствуют. Execution evidence отсутствует.

**Следом:** закрыть `COMP-005`/`JSON-002` — удалить либо version-ограничить legacy adapter trust report API. Параллельно остаются `MCP-004`, transactional Index cancellation review, `DOC-003` и Rust-enabled verification.

## Журнал

### 2026-07-17 — Direct Docs proposal composition

- Docs propose-fix перенесён в bounded `docs_direct/propose.rs` и загружает canonical snapshot через `composition.init_store`.
- `application_report_composition.rs` больше не импортирует и не вызывает `with_store_composition`; task-local service bridges в API/Docs/Repair direct paths отсутствуют.
- Существующий Docs implementation blob сохранён как `docs/legacy_impl.rs`; компактный `docs.rs` раскрывает только crate-private builder от уже загруженного snapshot.
- `service_composition_inventory.rs` проверяет direct routing, отсутствие global/task-local Store lookup и ограничения размеров модулей.
- `COMP-008` отмечен `[x] implemented`; execution остаётся pending. Для удаления compatibility include добавлен `DOC-003`.

### 2026-07-17 — Direct Docs check, drift and apply composition

- Добавлены bounded `docs_direct/snapshot.rs`, `docs_direct/check.rs` и `docs_direct/apply.rs`.
- Docs check и drift загружают canonical snapshot через `composition.init_store`; check сохраняет policy, severity и scope filtering, drift использует canonical report builder.
- Docs apply-patch напрямую валидирует snapshot identity, safe project paths и frontmatter updates без task-local Store scope.
- `application_report_composition.rs` использует `with_store_composition` только для Docs propose-fix.
- `service_composition_inventory.rs` требует ровно один scoped service call и ограничивает размеры direct modules.
- `COMP-008` остаётся in progress только из-за proposal builder и отсутствующего execution evidence.
...
- `service_composition_inventory.rs` запрещает возврат API/Repair к task-local/global store lookup.
