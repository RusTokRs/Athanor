...
| 3.5 | частично выполнен | Tokio process runner supports bounded streams, timeout, explicit manifest-directory `current_dir`, typed cancellation, and cancellable indexing propagates token into process source/extractor/linker/checker phases. On Windows timeout/cancel invokes `taskkill /T /F` before direct-child fallback; on Unix each adapter runs in a dedicated process group and timeout/cancel sends group `TERM` then `KILL` before that fallback. `athanor-core` exposes transport-neutral ProcessRunner request/output contract and `TokioProcessRunner` implements it; Windows Job Objects remain. |
| 4.1 | частично выполнен | manifest trust, allowlist и executable SHA-256 binding есть; trust registry v2 теперь дополнительно связывает canonical executable path, digest и byte size (v1 требует explicit re-trust), а assembled external command повторно сверяет digest/size непосредственно перед launch. Добавлен opt-in `external_process_sandbox = "clean_environment"`, очищающий inherited env перед process launch; `ath config doctor` показывает выбранный profile и его границы. network/filesystem/CPU sandbox остаются. |
| 4.2 | частично выполнен | cancellable pipeline scopes `CancellationToken` around source discovery, extraction, linking, and checking, so external adapter subprocesses receive it through Tokio task-local context. `athanor-core::OperationContext` теперь несёт optional operation id/deadline и передаётся в source/extractor/linker/checker и canonical store context-aware ports; pipeline применяет Tokio timeout к adapter и store begin/write/commit boundaries, а `index_project_with_operation_context` и cancellable counterpart позволяют host передать контекст без изменения `IndexOptions`. Daemon index/generate/wiki/HTML commands принимают optional future `deadline_unix_ms` и передают его в этот context, сохраняя legacy запрос без поля. Bounded read-only daemon commands `overview`/`explain`/`search`/`context`/`change-map` также принимают optional future deadline и ограничивают async request work через Tokio timeout. MCP tools принимают тот же optional field и transport ограничивает ожидание выполнения через Tokio timeout, сохраняя legacy payload без поля. Cancellable generation/wiki/HTML services также принимают context и кооперативно проверяют deadline в projector callback; жёсткое прерывание arbitrary synchronous projector factory и contract matrix остаются. Rollback намеренно выполняется вне deadline. |
...
| 4.5 | частично выполнен | `abort_snapshot` есть во всех storage adapters, pipeline вызывает rollback при write/commit failure; pipeline передаёт все canonical objects через единый `SnapshotBatch`/`put_snapshot` storage boundary. Memory/transient/JSONL применяют batch под одним snapshot-state lock, а `AthanorStore` сохраняет native delegation; compatibility path других backend остаётся sequential. После batch pipeline вызывает `prepare_snapshot`; JSONL materializes hidden prepared directory и commit atomically promotes it перед latest pointer. `[pipeline]` ограничивает batch по `max_snapshot_batch_objects` и `max_snapshot_batch_bytes` до storage call. Portable transaction-handle protocol across all backend и отдельный aggregate memory budget остаются. |
| 4.6 | частично выполнен | Добавлена явная immutable `RuntimeComposition`; composition-aware APIs покрывают index/update/generate, context, explain, overview, impact, coverage, capabilities, change-map, check, wiki, HTML report, generation и search без global factory lookup. `athd serve` передаёт composition в daemon state и использует её в index/generate/wiki/HTML/snapshot/search/context/change-map jobs; compatibility `OnceLock` для legacy registry изолирован в `runtime/legacy_registry.rs`. Внешний `TokioProcessRunner` пока не является composition dependency: текущий core port намеренно не принимает application cancellation, поэтому для DI требуется отдельный cancellation-aware application port. |
...
| 4.8 | частично выполнен | explicit composition вынесена в `composition.rs`, async `TokioProcessRunner` — в `runtime/process_runner.rs`, а external process adapter constructors, trait implementations, bounded execution/support — в `runtime/process_adapter.rs` и `runtime/process_adapter_support.rs`; plugin discovery/trust/hash/status — в focused `runtime/plugin_*.rs` modules, pipeline phases — в `pipeline_source.rs`, `pipeline_extract.rs`, `pipeline_link.rs`, `pipeline_check.rs`, а merge/metrics/ownership/support — в отдельные focused modules; runtime cancellation regression теперь использует тот же `process_adapter` execution path, без дублирующей test-only реализации. daemon client transport — в `daemon_client.rs`, server connection I/O/busy rejection — в `daemon_connection.rs`, protocol responses/limits, request-shape/deadline и authentication-policy validation — в `daemon_protocol.rs`, shared write-job operation id/deadline construction — в `daemon_operation.rs`, canonical snapshot/query cache/search handle и cached read-only execution — в `daemon_queries.rs`, endpoint I/O — в `daemon_endpoint.rs`, watcher/debounce — в `daemon_watcher.rs`, read-only job registry — в `daemon_job_registry.rs`, lifecycle/active-job state, shutdown cancellation и bounded drain — в `daemon_lifecycle.rs`, поиск/удаление cancellation-token и переходы отмены — в `daemon_job_cancellation.rs`, выделение queued-job и регистрация cancellation-token — в `daemon_job_scheduler.rs`, running/terminal job state — в `daemon_job_state.rs`, detached write-job execution/terminal reporting — в `daemon_write_jobs.rs`, чистые job helpers — в `daemon_jobs_support.rs`, invalidation — в отдельный module, daemon query `BoundedCache` — в `daemon_runtime.rs`. `runtime.rs` и daemon command orchestration всё ещё требуют дальнейшей декомпозиции. |
| 4.9 | частично выполнен | core имеет stable `CoreErrorCode`, retryability и typed conflict/snapshot/busy/cancel/deadline categories, включая distinct `adapter_protocol` для malformed JSON output external adapter; daemon v3 публикует endpoint и structured `error_details` (code/retryable/message/details), сохраняя legacy строку `error` и v2 endpoint/request/response compatibility. MCP сохраняет JSON-RPC `-32603`, но теперь публикует stable `error.data.code`/`retryable` для core categories, а unexpected internal errors получают generic message без raw path/stderr. Матричный unit test фиксирует mapping каждой `CoreError` category в daemon code/retryability; полный e2e contract matrix остаётся. |
...
- [-] 3.5: внешний adapter runner переведён со `std::process` и блокирующих потоков на
  ...
  ProcessRunner port и Windows Job Objects; Unix adapter processes получают отдельную process group,
  которой timeout/cancellation посылают `TERM`, затем `KILL`, до direct-child fallback.
