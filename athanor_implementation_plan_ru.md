...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP lifecycle/composition, adapter contracts, publication hardening, JSONL store и bounded CLI root реализованы; Docs proposal composition, execution и remaining cleanup pending |
...
| `COMP-008` | P2 | `[-] in progress` | API/Docs/Repair facades использовали task-local store composition вокруг legacy service internals | API snapshot/registry, Repair recovery/latest и Docs check/drift/apply переведены на прямой `composition.init_store`; перенести только Docs propose-fix builder, выполнить behavioral parity/isolation regressions и удалить последний task-local bridge |
...
- [x] Docs check/drift/apply разделены на bounded `docs_direct/{snapshot,check,apply}.rs` и вызывают `composition.init_store` напрямую.
...
- [x] Source regressions защищают active routing, direct services, MCP composition и единственный оставшийся Docs proposal bridge.

Осталось:

- [ ] Перенести Docs propose-fix builder с task-local scope на прямые service internals (`COMP-008`).
...
**Сейчас:** `COMP-008` остаётся `[-] in progress`: API snapshot/registry, Repair recovery/latest и Docs check/drift/apply переведены на direct composition; только Docs propose-fix всё ещё использует task-local Store scope. Execution evidence отсутствует.

**Следом:** отделить большой Docs proposal builder от legacy `docs.rs` owner и удалить последний `with_store_composition` service bridge. Параллельно остаются behavioral parity/isolation execution, `MCP-004`, transactional Index cancellation review и Rust-enabled verification.
