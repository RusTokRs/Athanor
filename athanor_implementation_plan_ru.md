...
| `DS-JSON-001` | P1 | `[-] in progress` | Registry содержит 17 golden-protected contracts; Search/Impact builders мигрированы; два compatibility blockers остаются |
| `DS-JSON-002` | P1 | `[-] in progress` | Inventory создан, bounded enforcement и shared-constant regressions включены для мигрированных builders |
...
- [x] Source-level regression запрещает возврат quoted schema literals в мигрированные Search/Impact builders.
...
- [ ] Перевести Diagnostic/Affected/Operations Docs Check и ChangeMap builders на shared schema constants.
...
**Сейчас:** `DS-JSON-001/002` — Search и Impact builders используют shared constants; Check family и ChangeMap остаются в literal-migration срезе.

**Дальше:** завершить Check/ChangeMap literal migration, затем typed Context contract и разделение Project Registry schemas до transport parity.
...
- Literal-migration остаётся `in progress` до Check family и ChangeMap.
