...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter contracts, explicit composition, publication hardening, JSONL store и focused CLI modularization реализованы; execution и remaining cleanup pending |
...
| `CLI-001` | P2 | `[-] in progress` | `entry.rs` всё ещё включает monolithic `main.rs`; root grammar и часть renderers дублируются | Один root command model, no `main.rs` include, no legacy bridges, bounded modules |
...
- [x] Удалены `direct_read_cli.rs`, `direct_read_composed_cli.rs`, `direct_application_report_composed_cli.rs`, `repair_cli.rs`, `repair_composed_cli.rs`.
- [x] Source enforcement запрещает family `include!`, namespace shadowing и рост focused production modules выше заданных лимитов.

Осталось:

- [ ] Вынести Graph/RusTok/Check/API diff renderers из `legacy` namespace.
- [ ] Построить единый root command model для legacy-only command families.
- [ ] Удалить `include!("main.rs")`, `main.rs` и все legacy bridges.
...
**Сейчас:** `CLI-003` отмечен `[x] implemented`: Direct Read, Application Report и Repair больше не используют include wrappers или namespace shadowing; parser/execution/rendering разделены на bounded modules. Execution evidence отсутствует.

**Следом:** продолжить `CLI-001` — вынести Graph/RusTok/Check rendering, построить root command model для legacy-only families и удалить единственный оставшийся `include!("main.rs")`. `RUST-001`, `RUST-002` и `COMP-005` остаются ограничены `TOOL-001`.
...
- `CLI-003` отмечен `[x] implemented`; `CLI-001` остаётся in progress из-за root `main.rs` include и remaining render bridges.
