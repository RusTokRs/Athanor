...
| `PUB-002` | P1 | `[-] in progress` | Staged-replace helpers смешивали durable success и post-commit maintenance failure | App-level writers исправлены; закрыть `PUB-005` и выполнить inventory/failure regressions |
...
| `PUB-004` | P1 | `[x] implemented` | Read-model/index-state finalizers, project registry и publication journal возвращали backup cleanup failure после commit point | Все четыре owner логируют warning и сохраняют successful publication; source regression в `main`; execution pending |
...
| `TOOL-001` | P2 | `[!] blocked` | Connector позволяет полную замену проверяемых blob’ов, но не точечный patch особенно больших `runtime.rs`/JSONL store files; hosted workflows не дали наблюдаемого run | Получен безопасный patch/local-file action либо полностью проверяемые blob’ы для оставшихся больших файлов |
...
- [x] Read-model/index-state finalizers, project registry и publication journal больше не возвращают backup cleanup failure после commit point.
- [x] Source-regression защищает warning-path и отсутствие прежних propagating cleanup errors для всех четырёх `PUB-004` owners.
...
**Сейчас:** `PUB-004` отмечен `[x] implemented`: read-model/index-state finalizers, project registry и publication journal используют best-effort cleanup после commit point. Source-regression находится в `main`; execution evidence отсутствует.

**Следом:** `PUB-005` — исправить generation-bearing latest pointer и legacy latest/snapshot-sequence helpers в `athanor-store-jsonl` через безопасно проверяемые полные blob’ы. `RUST-001`, `RUST-002` и `COMP-005` остаются ограничены `TOOL-001`.
