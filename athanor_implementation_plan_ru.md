...
| `PUB-003` | P2 | `[-] in progress` | Не было системных failure-injection regressions для stage/publish/rollback/cleanup | Stage failure, Drop cleanup и target-race tests находятся в `main`; добавить rollback/post-commit cleanup/recovery matrix и выполнить весь набор |
...
- [x] Добавлены failure regressions: staged build failure сохраняет предыдущий target, unpublished immutable candidate очищается через `Drop`, target race сохраняет incumbent.
...
- [-] `PUB-003`: добавить rollback restoration, injected post-commit cleanup warning и выполнить recovery/retention fault matrices.
- [ ] Выполнить publication inventory, projector, app, JSONL и workspace regressions.
...
**Сейчас:** `PUB-003` находится `[-] in progress`. В `main` добавлены три failure regressions для stage failure, Drop cleanup и target race; execution evidence отсутствует.

**Следом:** завершить `PUB-003` rollback/post-commit cleanup/recovery matrix. `PUB-004`, `PUB-005`, `RUST-001`, `RUST-002` и `COMP-005` остаются ограничены `TOOL-001` до появления безопасного точечного patch path или наблюдаемой hosted execution.
