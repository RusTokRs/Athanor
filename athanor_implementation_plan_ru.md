...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter contracts и пять explicit-composition slice реализованы; execution/cleanup pending |
...
| `COMP-006` | P2 | `[-] in progress` | Focused handlers создают composition, но часть всё ещё вызывает legacy `athanor_runtime_defaults::install()` | Search/Context/Check/Graph/ValidateChanged/RusTok/Generation очищены; Direct Read и Application Report migrated/classified |
...
- [x] Focused Search, Context, Check, Graph, ValidateChanged, RusTok и Generation не вызывают global `install()`.
- [x] Source regression защищает facade routing и composition-only handlers, включая Generation.

Осталось:

- [ ] Закрыть `COMP-006`: Direct Read уже вызывает composition-aware operations, но global install ещё не удалён; Application Report требует composition-aware API snapshot/docs propose-fix paths.
- [ ] Выполнить Rustok/Generation/source и executable regressions.
...
**Сейчас:** Direct Generation (`wiki`, `report html`, `generate`) использует только `RuntimeComposition`; global runtime install удалён, source regression добавлен. Implementation находится в `main`, execution evidence отсутствует.

**Следом:** закончить `COMP-006` — удалить лишний install из Direct Read и добавить composition-aware API snapshot/docs propose-fix paths для Application Report; затем безопасный repair `runtime.rs`, legacy trust API cleanup и publication inventory.
