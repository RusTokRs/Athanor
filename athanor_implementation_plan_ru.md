...
ChangeMap, Overview, Capabilities, Impact, Coverage и Check физически разделены на conventional bounded
...
1. `graph.rs`;
2. `api.rs` snapshot owner;
3. public `store::init_store` facade после migration всех callers.
...
#### 3.3 Store facade removal

После migration `graph.rs` и `api.rs`:
...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | Check/Repair/Docs/Graph operation закрыты; graph/api/Store pending |
...
- [-] Graph project reads и API snapshot ещё имеют hidden Store path.
...
- Public report/options model перенесён в `check/model.rs`.
- Mandatory composition loading и три execution entrypoint перенесены в `check/execution.rs`.
- Diagnostic scope policy, API gates, severity counts и deterministic sorting перенесены в
  `check/diagnostics.rs`.
- Affected-file matching, stale artifact inspection и documentation drift перенесены в
  `check/affected.rs`.
- Regressions перенесены и консолидированы в `check/tests.rs`.
- Удалены `check_project`, `check_affected`, `check_operations_docs`, optional composition,
  `crate::store::init_store` и fallback branches.
- Добавлен отдельный `check_composition_inventory` с routing и line budgets.
...
- Latest/recovery files оставлены владельцами public option/report models и current contracts.
