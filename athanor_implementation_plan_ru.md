...
| `DS-JSON-003` | P1 | `[-] in progress` | Context, Index, Generation, Wiki, HTML и Config typed parity реализованы; verification pending |
...
- [ ] ApiSnapshot/DocsProposeFix wrappers и Repair application outputs/state.
...
- [x] Executable Config Validate/Doctor contract regression добавлен; запуск pending.
...
Осталось **3 implementation-пакета и 1 verification-пакет**; Config application и direct CLI migration завершены по реализации:

1. Remaining application outputs: ApiSnapshot, DocsProposeFix и Repair reports/state.
...
По объёму реализации осталось ориентировочно **16–25% DS-JSON работы**. Подтверждённая verification отстаёт сильнее, потому что Rust toolchain и hosted status сейчас недоступны.
...
**Сейчас:** `DS-JSON-001/002/003` — 45 public contracts; typed Config application owners и direct CLI dispatch реализованы; executable regression добавлен; verification pending.

**Дальше:** versioned `ApiSnapshotReport` и `DocsProposeFixReport`, затем Repair application outputs/state, после чего transport и process/persistence boundaries.
...
- `ath config validate --json` теперь сериализует `ConfigValidateReport` с flattened effective config.
- `ath config doctor --json` теперь сериализует `ConfigDoctorReport` вместо ad-hoc `serde_json::Value`.
...
- Локальный запуск regression, fmt и Clippy не выполнен: Rust toolchain и исходящий network в среде отсутствуют; hosted status через connector не виден.
