...
| `DS-JSON-001` | P1 | `[-] in progress` | Registry содержит 35 golden-protected public contracts; IndexReport blocker закрыт |
| `DS-JSON-002` | P1 | `[-] in progress` | Scanner различает public, embedded, generated и persisted schemas |
| `DS-JSON-003` | P1 | `[-] in progress` | Context и typed Index parity реализованы; executable parity остальных operations остаётся |
...
- [x] `IndexReport` custom serializer добавляет top-level `athanor.index_report.v1` и сохраняет все прежние CLI fields.
- [x] Direct CLI Index/Update автоматически сериализуют versioned `IndexReport` без call-site wrappers.
- [x] Daemon index job сохраняет полный `IndexReport` вместо reduced hand-built object.
- [x] Все прежние daemon index fields сохранены; root/validation/validate-only fields добавлены аддитивно.
...
- [x] Fixture защищает nested IndexReportMetrics и IndexPipelineMetrics shapes.
- [x] Daemon unit regression сравнивает job result с direct IndexReport serialization.
- [x] Registry unit regressions защищают 35 schema ids и ownership.
...
- [x] Зафиксированы 35 public contracts и их Rust owners.
- [x] Scanner охватывает public read/project/Rustok plus benchmark/index/pipeline/changed-validation modules.
...
- [x] Direct CLI Index/Update и daemon index job используют один `IndexReport` serialization shape.
- [x] Unit regression сравнивает daemon job payload с direct serialization.
- [ ] Добавить executable CLI/daemon Index parity regression.
...
**Сейчас:** `DS-JSON-001/002/003` — 35 public contracts; typed Index CLI/daemon parity реализована; verification pending.

**Дальше:** config/API/docs/generation/wiki/HTML/repair JSON outputs, затем daemon/MCP envelopes и process/persistence inventory.
...
- Registry вырос с 34 до 35 public contracts.
