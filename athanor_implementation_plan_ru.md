...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter current contracts и два composition slice реализованы; execution/cleanup pending |
...
| `COMP-001` | P2 | `[x] implemented` | `OnceLock::set` conflicts молча игнорировались | Adapter/projector/Store/Search facades дают explicit conflict errors; execution pending |
...
| `COMP-004` | P2 | `[x] implemented` | `validate_changed` использовал `RuntimeBuilder::new` и hidden global adapter composition | Composition-aware API и active CLI dispatcher находятся в `main`; execution pending |
...
| `COMP-006` | P2 | `[-] in progress` | Focused CLI handlers создают explicit composition, но часть всё ещё вызывает legacy `athanor_runtime_defaults::install()` | Search/Context/ValidateChanged очищены; остальные handlers классифицированы и migrated |
...
- [x] Store factory скрыт за guarded facade с typed install/unavailable errors.
- [x] Normal и operation-aware Search factories скрыты за guarded facade.
- [x] `validate_changed_with_composition` использует `RuntimeBuilder::from_composition`.
- [x] Active `validate-changed` CLI переведён на focused composition-aware dispatcher.
- [x] Direct Search и Context больше не вызывают legacy global `install()`.
- [x] Source-level regression защищает facade routing, typed failures и composition-aware CLI paths.
...
- [ ] Закрыть `COMP-006`: удалить legacy `install()` из остальных composition-complete focused handlers.
...
**Сейчас:** все найденные проблемы сведены в единый реестр. Adapter/projector/Store/Search factories имеют guarded public paths; active Search/Context/ValidateChanged CLI используют explicit composition без global install.

**Следом:** классификация и очистка остальных focused CLI handlers, затем безопасный repair большого `runtime.rs`, legacy trust API cleanup и publication inventory.

## Журнал

### 2026-07-17 — ARCH-AUDIT-001 second composition slice

- Store и Search globals помещены за guarded facades с typed conflict/unavailable errors.
- Добавлен composition-aware `validate_changed` и focused CLI dispatcher.
- Direct Search/Context/ValidateChanged больше не мутируют global runtime.
- Оставшиеся focused handlers с legacy `install()` выделены как `COMP-006`.
