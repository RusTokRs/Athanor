...
| `PUB-003` | P2 | `[-] in progress` | Не было системных failure-injection regressions для stage/publish/rollback/cleanup | Stage/build, rollback, Drop cleanup, target-race и recovery/retention matrix находятся в `main`; добавить post-commit cleanup injection и выполнить набор |
...
- [-] `PUB-003`: добавить injected post-commit cleanup warning и выполнить rollback/recovery/retention fault matrices.
...
**Сейчас:** `PUB-003` находится `[-] in progress`. В `main` добавлены staged-build, rollback-restoration, Drop-cleanup и target-race regressions, а также source-enforced recovery/retention matrix; execution evidence отсутствует.

**Следом:** добавить injected post-commit backup cleanup failure, доказывающий successful publication + warning. `PUB-004`, `PUB-005`, `RUST-001`, `RUST-002` и `COMP-005` остаются ограничены `TOOL-001` до появления безопасного точечного patch path или наблюдаемой hosted execution.
