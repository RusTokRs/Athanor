...
| --- | ---: | --- |
| Implemented | 34 | Код и source/behavior regressions находятся в `main`; execution часто pending |
...
1. `COMP-003B2/C` — вынести Store/Search globals из default build, затем удалить no-composition bootstrap APIs.
...
`COMP-003A` и `COMP-003B1` завершены на уровне implementation: active entrypoints composition-only; task-local Store bridge удалён; adapter/projector globals вынесены в feature/test-only owners. Крупная новая архитектурная работа теперь сосредоточена в Store/Search feature quarantine и `MCP-007`.
...
| `COMP-003` | P2 | `[-] in progress` | Store/Search/adapter/projector globals сохраняли process-wide hidden dependencies | `A` и `B1` implemented; Store/Search feature quarantine и final wrapper removal остаются |
...
| `COMP-003B1` | P2 | `[x] implemented` | Adapter/projector globals компилировались в default production build | `legacy-global-runtime` feature, disabled default owners, feature/test-only `OnceLock`, CI/source enforcement; execution pending |
| `COMP-003B2` | P2 | `[-] in progress` | Store/Search globals всё ещё находятся в default implementation owners | Factory storage вынесено в feature/test-only bounded modules; default paths fail-fast without globals |
...
- [x] `runtime.rs` сокращён с 1846 до conventional module root.
...
- [x] Wiki/HTML `OnceLock` перенесены из `projection.rs` в `projection_legacy_global.rs`.
- [x] Default build содержит explicit fail-fast paths без process-global projector storage.
- [x] `athanor-runtime-defaults` forward-ит `legacy-global-runtime` в `athanor-app`.
- [x] CI feature matrix содержит отдельный legacy slice и `all-features`.
...
**Сейчас:** `COMP-003B2` — вынести Store и Search factory storage из `store.rs`/`search.rs` и их дублирующих facades в отдельные bounded-модули, компилируемые только при `legacy-global-runtime` или `cfg(test)`.

Требования:

- `store.rs` и `search.rs` не содержат `OnceLock` и installer functions;
- единственный Store global owner и единственный Search global owner;
- default build не содержит Store/Search process-global storage;
- no-composition wrappers default-build возвращают actionable disabled errors;
- unit tests сохраняют controlled bootstrap через `cfg(test)`;
- default, legacy feature и all-features checks находятся в CI/source inventory.

Ограничение исполнения: connector не поддерживает plaintext patch больших Rust files, поэтому перенос выполняется атомарной Git tree заменой после полного source reconstruction и статической сверки.
