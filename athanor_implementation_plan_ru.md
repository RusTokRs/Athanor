...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter contracts и три explicit-composition slice реализованы; execution/cleanup pending |
...
| `COMP-006` | P2 | `[-] in progress` | Focused handlers создают composition, но часть всё ещё вызывает legacy `athanor_runtime_defaults::install()` | Search/Context/Check/Graph/ValidateChanged очищены; остальные handlers migrated/classified |
| `COMP-007` | P2 | `[ ] planned` | RusTok operation APIs напрямую используют global `init_store`/global context operation и не принимают composition | Добавлены composition-aware Rustok audit/graph/context APIs; focused RusTok не вызывает global install |
...
- [x] Focused Search, Context, Check, Graph и ValidateChanged не вызывают global `install()`.
- [x] Source regression защищает facade routing и composition-only handlers.

Осталось:

- [ ] Закрыть `COMP-006`: Direct Read/Application Report/Generation и другие handlers классифицировать по фактической зависимости.
- [ ] Закрыть `COMP-007`: Rustok operation family.
...
**Сейчас:** все найденные проблемы имеют ID. Adapter/projector/Store/Search factories guarded; Search/Context/Check/Graph/ValidateChanged focused paths используют explicit composition.

**Следом:** Rustok composition family (`COMP-007`), затем remaining focused handlers, безопасный repair `runtime.rs`, legacy trust API cleanup и publication inventory.
