...
1. `COMP-003` — удалить оставшиеся process-global compatibility factories после caller/parity inventory.
...
Крупная новая архитектурная работа теперь сосредоточена в `COMP-003` и `MCP-007`. Verification может открыть дополнительные defects; они должны получать новые ID, а не скрываться внутри существующих статусов.
...
| `COMP-003` | P2 | `[-] in progress` | Store/Search/adapter/projector globals сохраняют process-wide hidden dependencies | Caller inventory, parity coverage и удаление неиспользуемых global factories |
...
**Сейчас:** `COMP-003` — инвентаризировать оставшиеся Store/Search/adapter/projector global factories, классифицировать реальные external compatibility callers и удалить globals, которые больше не нужны active entrypoints.

Требования:

- source inventory для всех install/default/init access points;
- active application/CLI/MCP code не обращается к global factories;
- compatibility API либо удалён, либо изолирован и явно deprecated;
- parallel composition isolation regressions;
- после удаления — targeted tests и Clippy.
