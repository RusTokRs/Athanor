...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter contracts, explicit composition, publication hardening, JSONL store и focused CLI/root routing реализованы; execution и remaining cleanup pending |
...
| `COMP-008` | P2 | `[-] in progress` | Application Report и Repair service facades временно используют task-local store composition вокруг legacy internals | CLI wrappers удалены; выполнить isolation regression, внедрить прямые service composition APIs и удалить task-local bridge |
...
| `CLI-001` | P2 | `[-] in progress` | `entry.rs` всё ещё включает monolithic `main.rs`; legacy-only grammar остаётся внутри файла | Удалены `include!("main.rs")` и legacy file; все команды принадлежат bounded root/family models; parser/help/output parity выполнены |
...
| `TOOL-002` | P2 | `[!] blocked` | Contents connector требует полную замену 4k-line `apps/ath/src/main.rs`, поэтому однострочное безопасное export-изменение нельзя выполнить атомарным patch | Получен patch-capable path либо legacy commands перенесены полностью и файл удалён без compatibility shim |
...
- [x] Application Report и Repair CLI напрямую вызывают composition-aware facades без namespace wrappers.
...
- [x] Application Report напрямую вызывает composition-aware APIs.
...
- [x] `root_command.rs` является единым типизированным владельцем порядка focused parser dispatch.
- [x] Из `entry.rs` удалены все renderer bridges.
- [x] Source enforcement запрещает family `include!`, namespace shadowing, renderer bridges и рост focused production modules выше заданных лимитов.

Осталось:

- [ ] Перенести legacy-only command families из `main.rs` в root/family models.
- [ ] Удалить единственный `include!("main.rs")` и затем удалить `main.rs`.
- [ ] Выполнить parser, help, JSON/text rendering и executable parity regressions.
...
cargo test -p ath -- direct_application_report_cli --locked
...
**Сейчас:** `CLI-004` отмечен `[x] implemented`: Graph/RusTok/Check/API-diff rendering отделён от legacy namespace, RusTok разбит на bounded modules, а focused routing принадлежит одному typed root command model. Execution evidence отсутствует.

**Следом:** продолжить `CLI-001` — перенести legacy-only grammar/dispatch из `main.rs` и удалить последний `include!("main.rs")` без compatibility shim. Для точечного export-шага текущий connector ограничен `TOOL-002`; допустимый альтернативный путь — полный перенос оставшихся families. После этого перейти к `COMP-008`.
...
- Добавлен typed `root_command::Command`, который единолично задаёт порядок focused parsers и явный временный legacy fallback.
- Из `entry.rs` удалены все 13 renderer bridge functions.
- `legacy_factory_migration.rs` проверяет root routes, отсутствие legacy imports, bounded modules и отсутствие family includes.
- `CLI-004` отмечен `[x] implemented`; `CLI-001` остаётся in progress только из-за legacy-only grammar и последнего `main.rs` include.
...
- Удалены три composition/include wrappers и два monolithic command bodies общей длиной более 1100 строк.
- `legacy_factory_migration.rs` запрещает family includes, namespace shadowing и unbounded focused modules.
- `CLI-003` отмечен `[x] implemented`; `CLI-001` остаётся in progress из-за root `main.rs` include.
- Rust execution evidence отсутствует.
...
- Добавлены composition-aware Graph, RusTok, Application Report и Repair paths.
- `COMP-006` отмечен `[x] implemented`; `COMP-008` остаётся in progress до удаления task-local service bridge.
