...
| --- | --- | --- | --- |
| `ARCH-AUDIT-001` | P1 | `[-] in progress` | MCP, adapter contracts, explicit composition, publication hardening и JSONL store modularization реализованы; execution и remaining cleanup pending |
...
| `TOOL-001` | P2 | `[!] blocked` | Connector не поддерживает точечный patch особенно большого `runtime.rs`; hosted workflows не дали наблюдаемого run | Получен безопасный patch/local-file action либо полностью проверяемый blob для оставшегося runtime файла |
...
- [x] Application Report и Repair используют scoped explicit composition.
...
- [ ] Разделять оставшиеся длинные CLI-файлы по command family и держать production-модули bounded.

### Documentation reconciliation

- [x] Затронутые adapter/publication contract docs приведены к честному current status.
...
**Сейчас:** `PUB-002`, `PUB-005` и `STORE-001` отмечены `[x] implemented`: все подтверждённые pointer writers используют единый commit/maintenance contract, JSONL include-chain удалён, production-модули bounded. Execution evidence отсутствует.

**Следом:** `CLI-001` — декомпозировать оставшийся CLI monolith и compatibility includes по тому же принципу: explicit modules, единый command model, bounded production files. `RUST-001`, `RUST-002` и `COMP-005` остаются ограничены `TOOL-001`.
...
- Project registry backup cleanup переведён на warning-path параллельным commit `4ea314ee…`, который принят в текущий baseline.
- Publication journal, index-state finalizer и read-model finalizer теперь не возвращают maintenance failure после успешного commit rename.
- `publication_semantics_inventory.rs` защищает warning messages и отсутствие прежних propagating cleanup errors для всех четырёх owners.
- Durability inventory синхронизирован; `PUB-004` отмечен `[x] implemented`.
...
- `COMP-006` отмечен `[x] implemented`; `COMP-008` остаётся in progress до удаления compatibility bridge.
