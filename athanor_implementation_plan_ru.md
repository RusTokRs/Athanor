...
| `DS-JSON-001` | P1 | `[-] in progress` | Versioned JSON registry, ownership checks и семь golden-protected v1 contracts в `main` |
...
- [x] Зарегистрировано семейство Check: `diagnostic_check`, `affected_check`, `operations_docs_check`.
- [x] Семь public report types реализуют общий contract trait.
...
- [x] Integration tests сериализуют реальные и public report types и сравнивают их с fixtures.
- [x] Check fixtures фиксируют `DiagnosticScope`, nested counts и nested check documents.
...
- [ ] Зарегистрировать Coverage/Capabilities reports.
- [ ] Инвентаризировать остальные внешние JSON documents в app, CLI, daemon, MCP и process-adapter protocols.
- [ ] Назначить одному Rust type единственного schema owner.
- [ ] Заменить оставшиеся локальные schema literals shared constants, включая Check builders.
...
**Сейчас:** `DS-JSON-001` — Overview, Search, Explain, Impact и Check family зарегистрированы и защищены fixtures; verification pending.

**Дальше:** Coverage/Capabilities, затем remaining inventory, shared-literal migration и parity tests.
