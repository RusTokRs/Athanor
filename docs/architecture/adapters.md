...
| `athanor-extractor-graphql` | `Extractor` | Extract standalone GraphQL SDL root schema/type, operation, fragment, directive, and introspection JSON files into shared API endpoint/schema or adapter-scoped fragment/directive knowledge with evidence-backed declarations and same-file validation diagnostics for unresolved fragment spreads, unresolved inline type conditions, and deprecated field usage. |
