...
It emits `implemented_by`, `documents_operation`, and `documents_api` inferred relations. Matches are deterministic and lexical: exact normalized `operationId` for Rust handlers at confidence 0.7, and operation id, final static path segment, or tags for documentation at confidence 0.5. It also resolves same-document OpenAPI component `$ref` values into verified `schema_for_request` and `schema_for_response` relations at confidence 1.0. Schema relation payloads retain media type and response status metadata. Every relation contains evidence and ownership from both sides and respects the affected subset during incremental indexing.
