---
entities:
- script-command://.github/workflows/ci.yml#github-actions:job:quality
id: doc://docs/operations/script-script-command-github-workflows-ci-yml-github-actions-job-quality.md
kind: operations_documentation
language: en
last_verified_snapshot: snap_jsonl_00000272
source_language: en
status: verified
---
# Script Command `GitHub Actions job Rust 1.95 / ${{ matrix.os }}`

## Purpose

Document when this operational item is used, who owns it, and what successful execution or operation means.

## Contract

- Canonical entity: `script-command://.github/workflows/ci.yml#github-actions:job:quality`
- Entity kind: `script_command`
- Source: `.github/workflows/ci.yml`:30

## Procedure

1. Review the source definition and confirm prerequisites.
2. Execute or operate this item according to the project runbook.
3. Record outcomes, rollback notes, and follow-up actions.

## Evidence

- `.github/workflows/ci.yml:30`

## Notes

Generated from diagnostic `diag_script_26071a40c92869cc`. Review this page before relying on it as operational documentation.
